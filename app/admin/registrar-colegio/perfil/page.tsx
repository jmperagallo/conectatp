    ...
    <div style={{ minHeight: "100vh", backgroundColor: COLORES.fondo, fontFamily: "system-ui, sans-serif", display: "flex", flexDirection: "column" }}>
      ...
      <main style={{ flex: 1, display: "flex", justifyContent: "center", padding: "40px 24px" }}>
        <form onSubmit={handleRegistrarEcosistema} style={{ width: "100%", maxWidth: "750px", display: "flex", flexDirection: "column", gap: "24px" }}>
          {/* Sección de seguridad y advertencia (igual que antes) */}
          ...
          <div style={{ backgroundColor: "white", borderRadius: "24px", padding: "30px", border: `1px solid ${COLORES.borde}`, boxShadow: "0 4px 10px rgba(0,0,0,0.02)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                <div><label style={labelStyle}>Correo Electrónico Principal (🔒 Bloqueado)</label><input type="text" style={getInputStyle(true)} value={correoPrincipal || "No detectado"} disabled /></div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "24px", alignItems: "center", background: "#f8fafc", padding: "20px", borderRadius: "16px", border: `1px dashed ${COLORES.borde}` }}>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "130px", backgroundColor: "white", borderRadius: "12px", border: `1px solid ${COLORES.borde}`, overflow: "hidden" }}>
                  {previewLogo ? <img src={previewLogo} alt="Insignia Colegio" style={{ width: "100%", height: "100%", objectFit: "contain", padding: "10px" }} /> : <div style={{ display: "flex", flexDirection: "column", alignItems: "center", color: COLORES.grisClaro }}><ImageIcon size={32} style={{ opacity: 0.5, marginBottom: "4px" }} /><span style={{ fontSize: "11px" }}>Sin Insignia</span></div>}
                </div>
                <div>
                  ...
                  <div onClick={() => fileInputRef.current?.click()} style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "20px", border: `2px dashed ${COLORES.naranja}`, borderRadius: "12px", backgroundColor: "#fffbfa", cursor: "pointer" }}>
                    ...
                    <span style={{ fontSize: "13px", fontWeight: "700", color: COLORES.azul }}>{uploadingLogo ? "Subiendo logo..." : "Haz clic para cargar imagen"}</span>
                  </div>
                </div>
              </div>
              <div><label style={labelStyle}>Decreto Cooperador / Res. Exenta Mineduc</label><input type="text" placeholder="Ej: Decreto N° 4250 / 1998" style={inputStyle} value={decretoCooperador} onChange={(e) => setDecretoCooperador(e.target.value)} /></div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                <div><label style={labelStyle}>Teléfono de Red Fija</label><input type="text" placeholder="Ej: +56 58 222 XXXX" style={inputStyle} value={telefonoFijo} onChange={(e) => setTelefonoFijo(e.target.value)} /></div>
                <div><label style={labelStyle}>Teléfono Móvil del Establecimiento</label><div style={{ display: "flex", gap: "12px", alignItems: "center" }}><input type="text" placeholder="+56 9 XXXX XXXX" style={{ ...inputStyle, flex: 2 }} value={telefonoMovilColegio} onChange={handleTelefonoColegioChange} /><label style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", fontWeight: "700", color: COLORES.texto, cursor: "pointer", userSelect: "none" }}><input type="checkbox" checked={tieneWhatsapp} onChange={(e) => setTieneWhatsapp(e.target.checked)} style={{ width: "16px", height: "16px", accentColor: "#16a34a" }} /> ¿WhatsApp?</label></div></div>
              </div>
              <div><label style={labelStyle}>Dirección Postal</label><div style={{ position: "relative" }}><MapPin size={16} color={COLORES.grisClaro} style={{ position: "absolute", left: "12px", top: "14px" }} /><input type="text" placeholder="Ej: Avenida Diego Portales #1230" style={{ ...inputStyle, paddingLeft: "36px" }} value={direccionPostal} onChange={(e) => setDireccionPostal(e.target.value)} /></div></div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", borderTop: `1px dashed ${COLORES.borde}`, paddingTop: "16px" }}>
                <div><label style={labelStyle}>Nombre de el/la Director(a)</label><input type="text" placeholder="Ej: Marta Silva Fuentealba" style={inputStyle} value={nombreDirector} onChange={(e) => setNombreDirector(e.target.value)} /></div>
                <div><label style={labelStyle}>Correo de el/la Director(a)</label><input type="email" placeholder="Ej: direccion@liceo.cl" style={inputStyle} value={correoDirector} onChange={(e) => setCorreoDirector(e.target.value)} /></div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                <div><label style={labelStyle}>Misión Institucional</label><textarea placeholder="Describe el sello educativo..." style={{ ...inputStyle, height: "100px", resize: "none" }} value={mision} onChange={(e) => setMision(e.target.value)} /></div>
                <div><label style={labelStyle}>Visión Institucional</label><textarea placeholder="Describe la proyección a futuro..." style={{ ...inputStyle, height: "100px", resize: "none" }} value={vision} onChange={(e) => setVision(e.target.value)} /></div>
              </div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: "white", borderRadius: "24px", padding: "30px", border: `1px solid ${COLORES.borde}`, boxShadow: "0 4px 10px rgba(0,0,0,0.02)" }}>
            ...
            <p style={{ fontSize: "13px", color: COLORES.grisClaro, margin: "0 0 20px 0" }}>Asigna el área técnica oficial del Mineduc para cada encargado de especialidad o mención del establecimiento.</p>
            <div style={{ background: "#f8fafc", padding: "20px", borderRadius: "16px", border: `1px solid ${COLORES.borde}`, display: "flex", flexDirection: "column", gap: "14px", marginBottom: "20px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1.25fr 1.25fr", gap: "12px" }}>
                <div><label style={labelStyle}>Nombres *</label><input type="text" placeholder="Ej: Carlos Alberto" style={inputStyle} value={jefeNombre} onChange={(e) => setJefeNombre(e.target.value)} /></div>
                <div><label style={labelStyle}>Apellido Paterno *</label><input type="text" placeholder="Ej: Retamales" style={inputStyle} value={jefeApPaterno} onChange={(e) => setJefeApPaterno(e.target.value)} /></div>
                <div><label style={labelStyle}>Apellido Materno *</label><input type="text" placeholder="Ej: Vega" style={inputStyle} value={jefeApMaterno} onChange={(e) => setJefeApMaterno(e.target.value)} /></div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: "12px" }}>
                <div>
                  <label style={labelStyle}>Seleccione Especialidad / Mención Oficial *</label>
                  <select style={inputStyle} value={jefeSelectorCompleto} onChange={(e) => {
                    const valor = e.target.value;
                    setJefeSelectorCompleto(valor);
                    if (!valor) { setJefeSector(""); setJefeEspecialidad(""); setJefeMencion(""); return; }
                    for (const sector in ESTRUCTURA_TP_CHILE) {
                      for (const especialidad in ESTRUCTURA_TP_CHILE[sector]) {
                        ...
                        if (menciones.length === 0 && valor === especialidad) {
                          setJefeSector(sector); setJefeEspecialidad(especialidad); setJefeMencion("No requiere"); return;
                        }
                        const encontrada = menciones.find(m => `${especialidad} - ${m}` === valor);
                        if (encontrada) { setJefeSector(sector); setJefeEspecialidad(especialidad); setJefeMencion(encontrada); return; }
                      }
                    }
                  }}>
                    ...
                    {Object.keys(ESTRUCTURA_TP_CHILE).map(sector => (
                      <optgroup key={sector} label={sector}>
                        {Object.keys(ESTRUCTURA_TP_CHILE[sector]).map(especialidad => {
                          const menciones = ESTRUCTURA_TP_CHILE[sector][especialidad];
                          if (menciones.length === 0) return <option key={especialidad} value={especialidad}>{especialidad}</option>;
                          return menciones.map(mencion => <option key={`${especialidad}-${mencion}`} value={`${especialidad} - ${mencion}`}>{especialidad} ({mencion})</option>);
                        })}
                      </optgroup>
                    ))}
                  </select>
                </div>
                <div><label style={labelStyle}>Correo Institucional Jefe *</label><input type="email" placeholder="Ej: jefe.informatica@liceo.cl" style={inputStyle} value={jefeCorreo} onChange={(e) => setJefeCorreo(e.target.value)} /></div>
              </div>
            ...
            <div style={{ overflowX: "auto", border: `1px solid ${COLORES.borde}`, borderRadius: "12px" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: "13px" }}>
                <thead><tr style={{ backgroundColor: "#f8fafc", borderBottom: `1px solid ${COLORES.borde}`, color: COLORES.grisClaro }}><th style={{ padding: "12px 16px" }}>Nombre Completo</th><th style={{ padding: "12px 16px" }}>Especialidad / Mención</th><th style={{ padding: "12px 16px" }}>Correo Electrónico</th><th style={{ padding: "12px 16px", width: "60px" }}></th></tr></thead>
                <tbody>
                  {listaJefes.length === 0 ? <tr><td colSpan={4} style={{ padding: "20px", textAlign: "center", color: COLORES.grisClaro, fontStyle: "italic" }}>Ningún jefe de especialidad asociado todavía.</td></tr> : listaJefes.map(j => (
                    <tr key={j.correo} style={{ borderBottom: `1px solid ${COLORES.borde}`, color: COLORES.texto }}>
                      ...
                      <td style={{ padding: "12px 16px" }}><span style={{ display: "block", fontWeight: "600", fontSize: "12px", color: COLORES.azul }}>{j.especialidad}</span><span style={{ fontSize: "11px", color: COLORES.grisClaro }}>{j.mencion}</span></td>
                      <td style={{ padding: "12px 16px" }}>{j.correo}</td>
                      <td style={{ padding: "12px 16px", textAlign: "center" }}>{j.correo !== correoPrincipal && <button type="button" onClick={() => handleQuitarJefe(j.correo)} style={{ border: "none", background: "none", padding: 4, cursor: "pointer", color: "#ef4444" }}><X size={16} /></button>}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "10px" }}>
            <button type="submit" disabled={saving} style={{ backgroundColor: COLORES.naranja, color: "white", border: "none", padding: "14px 28px", borderRadius: "14px", fontSize: "15px", fontWeight: "700", cursor: saving ? "not-allowed" : "pointer", boxShadow: "0 4px 12px rgba(249, 115, 22, 0.2)", opacity: saving ? 0.7 : 1 }}>{saving ? "Guardando cambios institucionales..." : "Guardar Ecosistema Completo"}</button>
          </div>
        </form>
      </main>
    </div>
